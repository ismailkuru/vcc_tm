...
//_(writes pathlist->resource)
{
	...
	_(invariant \forall unsigned j; j<i ==> \mutable(writeSet[j]) && \writable(writeSet[j]))
	...
	_(invariant onePath->path_len <= MAX_POINTS)
	_(invariant \forall unsigned j; j<onePath->path_len ==> onePath->xs[j]<WIDTH && onePath->xs[j]>=0)
	_(invariant \forall unsigned i; i<onePath->path_len ==> \mutable(writeSet[i]) && \writable(writeSet[i]))
	...
	_(assume \writable(pathlist->resource->pathlist))
	_(atomic c, _(read_only) pathlist){
		_(assume \writable(&pathlist->resource->pathlist[pathlist->resource->num_paths]))
...
void foo(PInt x, PInt y _(ghost \claim c))
_(always c, x->\closed && y->\closed && x != y)
{
	_(assume x->data == 3)
	_(assert x->data == 3)

	_(atomic c, y){
		_(assume \writable(&y->data))
		y->data = 4;
	}
	_(assert x->data == 3)
	//_(assert \false)
